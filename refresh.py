...
def refresh_data(rank = None):
    ...
    if rank is None:
        ...
        for df in rank_charts:
            rank_charts[df].save('rank_charts_' + df + '.json')
        for df in games_count_charts:
            games_count_charts[df].save('games_count_charts_' + df + '.json')
    else:
        ...
        # 导出为json文件
        rank_charts[rank].save('rank_charts_' + rank + '.json')
        games_count_charts[rank].save('games_count_charts_' + rank + '.json')
